...
learn-cli vendors its `.claude/skills/` from **guildmaster** — the
...
#   - consumer-identifying prose: `guildmaster` → `learn-cli` (NOT
...
If a re-sync would lose a learn-cli adaptation, lift the change
...
#     → `which learn-cli vendors from guildmaster`
