"""Smoke tests for the learn-cli CLI entry point and its verbs."""
...
from learn.explain import known_paths
...
def test_no_args_prints_help(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main([])
    assert rc == 0
    assert "usage: learn-cli" in capsys.readouterr().out
...
def test_explain_root(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main(["explain"])
    assert rc == 0
    assert "# learn-cli" in capsys.readouterr().out


def test_explain_self(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main(["explain", "learn-cli"])
    assert rc == 0
...
def test_explain_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "learn-cli whoami" in payload["markdown"]
