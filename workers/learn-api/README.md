...
153 tests cover session sign/verify/expiry, `recorded` validation (including the
...
approved. The Converse pass-through (`test/tutor-converse.test.js`, t15,
additive) proves the unchanged broker carries a native Bedrock Converse
payload verbatim — plus exactly the `learner` stamp — and relays the
Converse response shape and upstream error statuses untranslated. Tests
invoke the Worker's
