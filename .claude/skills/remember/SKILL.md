...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope learn-cli
...
scope:** the default personal scope is **private** (`--scope learn-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `learn-cli`). Private records are served only to a recall in the same
